...
def _pad_to_same_length(tensors):
    max_len = max(t.shape[1] for t in tensors)
    padded = []
    for t in tensors:
        pad_len = max_len - t.shape[1]
        if pad_len > 0:
            pad = torch.zeros((t.shape[0], pad_len, t.shape[2]), device=t.device, dtype=t.dtype)
...
class StableDiffusion():
    ...
    def get_text_embed(self, null_prompt, prompt):
        if self.compel is not None:
            ...
            uc, c = _pad_to_same_length([uc, c])
            return uc, c

        # fallback: standard 77-token encoding
