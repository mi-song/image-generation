...
import torch
from diffusers import AutoencoderKL, DDIMScheduler, EulerAncestralDiscreteScheduler, StableDiffusionXLPipeline, UNet2DConditionModel, EulerDiscreteScheduler
...
class SDXL():
    def __init__(self,
        ...
        # Support both HuggingFace model IDs and local safetensors/ckpt files
        ...
        # VAE selection: use_model_vae=True uses the pipeline's built-in VAE
        if kwargs.get('use_model_vae', False):
            self.vae = pipe.vae
        else:
        ...
        # Allow manual override (useful for models with incorrect metadata)
        if kwargs.get('prediction_type') is not None:
            self.prediction_type = kwargs['prediction_type']

        self.compel = None
        if kwargs.get('use_compel', False):
            try:
                from compel import Compel, ReturnedEmbeddingsType
                self.compel = Compel(
                    tokenizer=[self.tokenizer_1, self.tokenizer_2],
                    text_encoder=[self.text_enc_1, self.text_enc_2],
        ...
        # sampling parameters
        # Preserve original pipe scheduler config for schedulers that need it (e.g. euler_a_native)
        self.pipe_scheduler_config = pipe.scheduler.config

        # timestep_spacing: explicit kwarg > model config field > "trailing" (diffusers default for SDXL)
        _ts = kwargs.get('timestep_spacing', None)
        if _ts is None:
            _ts = getattr(pipe.scheduler.config, 'timestep_spacing', 'trailing')

        if ext in (".safetensors", ".ckpt"):
            self.scheduler = DDIMScheduler.from_config(pipe.scheduler.config, timestep_spacing=_ts)
        else:
            self.scheduler = DDIMScheduler.from_pretrained(model_key, subfolder="scheduler", timestep_spacing=_ts)
    ...
    def alpha(self, t):
        ...
        return self.total_alphas[t_val].to(self.device)
    ...
    @torch.no_grad()
    def _get_text_embed_compel(self, null_prompt, prompt):
        null_prompt = null_prompt if isinstance(null_prompt, str) else null_prompt[0]
        prompt = prompt if isinstance(prompt, str) else prompt[0]
        conditioning, pooled = self.compel(prompt)
        neg_conditioning, neg_pooled = self.compel(null_prompt)
        # pad to same length when they differ
        max_len = max(conditioning.shape[1], neg_conditioning.shape[1])
        for emb_list in [[conditioning, neg_conditioning]]:
            for i, e in enumerate(emb_list):
                if e.shape[1] < max_len:
                    pad = torch.zeros((e.shape[0], max_len - e.shape[1], e.shape[2]),
                                      device=e.device, dtype=e.dtype)
                    emb_list[i] = torch.cat([e, pad], dim=1)
            conditioning, neg_conditioning = emb_list
        return neg_conditioning, conditioning, neg_pooled, pooled
    ...
    def predict_noise(self, zt, t, uc, c, added_cond_kwargs):
        ...
        if self.prediction_type == "v_prediction":
            # Use total_alphas[t] directly (avoids off-by-1 from prepended alphas_cumprod)
            t_idx = t.long().clamp(0, len(self.total_alphas) - 1)
            at = self.total_alphas[t_idx].to(zt.device)
            noise_uc = at.sqrt() * noise_uc + (1 - at).sqrt() * zt
            noise_c = at.sqrt() * noise_c + (1 - at).sqrt() * zt

        return noise_uc, noise_c

    def predict_raw(self, zt, t, uc, c, added_cond_kwargs):
        """Raw UNet output with no v→ε conversion — for use with scheduler.step()."""
        t_in = t.unsqueeze(0)
        if uc is None:
            out_c = self.unet(zt, t_in, encoder_hidden_states=c,
                              added_cond_kwargs=added_cond_kwargs)['sample']
            out_uc = out_c
        elif c is None:
            out_uc = self.unet(zt, t_in, encoder_hidden_states=uc,
                               added_cond_kwargs=added_cond_kwargs)['sample']
            out_c = out_uc
        else:
            c_embed = torch.cat([uc, c], dim=0)
            z_in = torch.cat([zt] * 2)
            t_in = torch.cat([t_in] * 2)
            out = self.unet(z_in, t_in, encoder_hidden_states=c_embed,
                            added_cond_kwargs=added_cond_kwargs)['sample']
            out_uc, out_c = out.chunk(2)
        return out_uc, out_c
    ...
    def sample(self,
        ...
        if self.compel is not None:
            (null_prompt_embeds, prompt_embeds,
             pool_null_embed, pool_prompt_embed) = self._get_text_embed_compel(prompt1[0], prompt1[1])
        else:
            (null_prompt_embeds, prompt_embeds,
             pool_null_embed, pool_prompt_embed) = self.get_text_embed(prompt1[0], prompt1[1], prompt2[0], prompt2[1], clip_skip)
...
class Euler(SDXL):
    """Euler — native sigma schedule, t passed directly (matches diffusers)."""
    @torch.autocast(device_type='cuda', dtype=torch.float16)
    def reverse_process(self,
                        null_prompt_embeds,
                        prompt_embeds,
                        ...
                        callback_fn=None,
                        **kwargs):
        ts = self.scheduler.timesteps.cpu().long()
        alphas_at_ts = self.total_alphas[ts]
        sigmas = ((1 - alphas_at_ts) / alphas_at_ts).sqrt()
        ...
        pbar = tqdm(self.scheduler.timesteps.int(), desc='euler')
        for step, t in enumerate(pbar):
            sigma = sigmas[step]
...
class EulerA(SDXL):
    """
    Euler Ancestral — native sigma schedule (matches diffusers EulerAncestralDiscreteScheduler).
    """
    @torch.autocast(device_type='cuda', dtype=torch.float16)
    def reverse_process(self,
                        null_prompt_embeds,
                        ...
                        callback_fn=None,
                        **kwargs):
        # Native sigma schedule: same as diffusers (no Karras remapping)
        ts = self.scheduler.timesteps.cpu().long()
        alphas_at_ts = self.total_alphas[ts]
        sigmas = ((1 - alphas_at_ts) / alphas_at_ts).sqrt()
        sigmas = torch.cat([sigmas, sigmas.new_zeros([1])])   # append sigma=0
        ...
        pbar = tqdm(self.scheduler.timesteps.int(), desc='euler_a')
        for step, t in enumerate(pbar):
            sigma = sigmas[step]
            sigma_down, sigma_up = get_ancestral_step(sigma, sigmas[step + 1])

            with torch.no_grad():
                # Pass t directly (same timestep embedding as diffusers)
                z0t, _ = self.kdiffusion_zt_to_denoised(zt, sigma, null_prompt_embeds, prompt_embeds, cfg_guidance, t, add_cond_kwargs)
...
@register_solver('euler_a_native')
class EulerANative(SDXL):
    """
    Euler Ancestral using diffusers EulerAncestralDiscreteScheduler.step() directly.
    v→x0 conversion is handled inside the scheduler — matches diffusers pipeline output exactly.
    Use this as a reference to verify that custom samplers produce correct results.
    """
    @torch.autocast(device_type='cuda', dtype=torch.float16)
    def reverse_process(self,
                        null_prompt_embeds,
        ...
        native_sched = EulerAncestralDiscreteScheduler.from_config(
            self.pipe_scheduler_config,   # original model config, not DDIMScheduler-derived
            prediction_type=self.prediction_type,
        )
        n_steps = len(self.scheduler.timesteps)
        native_sched.set_timesteps(n_steps, device=self.device)

        zt_dim = (1, 4, shape[1] // self.vae_scale_factor, shape[0] // self.vae_scale_factor)
        zt = torch.randn(zt_dim, device=self.device, dtype=torch.float16)
        zt = zt * native_sched.init_noise_sigma

        pbar = tqdm(native_sched.timesteps, desc='euler_a_native')
        for step, t in enumerate(pbar):
            zt_input = native_sched.scale_model_input(zt, t)

            with torch.no_grad():
                out_uc, out_c = self.predict_raw(zt_input, t, null_prompt_embeds, prompt_embeds, add_cond_kwargs)
                noise_pred = out_uc + cfg_guidance * (out_c - out_uc)
...
class EulerCFGpp(SDXL):
    """Euler CFG++ — native sigma schedule, t passed directly."""
    @torch.autocast(device_type='cuda', dtype=torch.float16)
    def reverse_process(self,
                        null_prompt_embeds,
                        prompt_embeds,
        ...
        pbar = tqdm(self.scheduler.timesteps.int(), desc='euler_cfg++')
        for step, t in enumerate(pbar):
            sigma = sigmas[step]
...
class DPMpp2mCFGppSolver(SDXL):
    ...
    def reverse_process(self,
        ...
        # Native sigmas from total_alphas — consistent t/sigma pairing
        ts = self.scheduler.timesteps.cpu().long()
        alphas = self.total_alphas[ts]
        ...
        pbar = tqdm(self.scheduler.timesteps.int(), desc='dpm++_2m_cfgpp')
        for i, t in enumerate(pbar):
            sigma = sigmas[i]
            at = alphas[i].to(x.device)
            c_in = at.sqrt()
            c_out = -sigma

            with torch.no_grad():
                noise_uc, noise_c = self.predict_noise(x * c_in, t, null_prompt_embeds, prompt_embeds, add_cond_kwargs)
                noise_pred = noise_uc + cfg_guidance * (noise_c - noise_uc)
            ...
            if callback_fn is not None:
                callback_kwargs = {'z0t': denoised.detach(), 'zt': x.detach(), 'decode': self.decode}
